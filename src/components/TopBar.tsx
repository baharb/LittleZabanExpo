...
export default function TopBar({
  ...
  const titleSize = large ? Math.max(32, Math.min(width * 0.07, 48)) : 18;
  const titleSubSize = large ? Math.max(14, Math.min(width * 0.024, 17)) : 11;
  const localTitle = (() => {
  ...
  return (
    <View style={[styles.bar, { backgroundColor: bg, paddingTop: (Platform.OS === 'ios' ? 6 : 0) + topInset + (large ? 16 : 0), paddingBottom: large ? 18 : 8, paddingHorizontal: Math.max(12, Math.min(width * 0.022, 18)) }]}>
      {/* Back button — large, bold, pill-shaped */}
      ...
      <View style={styles.titleWrap}>
        {compactTitle ? (
          ...
          <>
            <Text style={[styles.titleFa, { color: txtColor, fontFamily: ff('en', 'black'), fontSize: titleSize, letterSpacing: 1.5 }]} numberOfLines={1}>
              {title || localTitle}
            </Text>
            {titleFa ? (
              <Text style={[styles.titleEnSub, { color: txtColor, fontFamily: ff('fa', 'bold'), fontSize: titleSubSize, opacity: 0.82 }]} numberOfLines={1}>
                {titleFa}
              </Text>
            ) : null}
          </>
...
const styles = StyleSheet.create({
  ...
  /* Large pill back button, bold chevron */
  backBtn: {
